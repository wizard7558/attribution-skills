...
SELECT CAST('synthetic-check' AS STRING) AS invocation_key, CAST('synthetic_crm' AS STRING) AS source_system, CAST('crm-a' AS STRING) AS source_scope, CAST('Record+A' AS STRING) AS record_key, TRUE AS included, TRUE AS eligible, FALSE AS meets_condition;
...
CREATE OR REPLACE TEMP TABLE decisions AS SELECT p.*,CASE WHEN NOT source_complete OR eligible_count=0 OR eligible_count<min_eligible_count THEN 'unknown'
 WHEN true_count>threshold*eligible_count THEN 'fail' WHEN (true_count+unknown_count)<=threshold*eligible_count THEN 'pass' ELSE 'unknown' END AS status,
...
CREATE OR REPLACE TEMP TABLE findings AS SELECT d.*,
 ...
 IF(status='fail','rate_above_maximum',NULL),IF(status='unknown' AND c.source_complete AND eligible_count>=c.min_eligible_count AND unknown_count>0,'condition_uncertainty',NULL)]) reason WHERE reason IS NOT NULL ORDER BY reason) AS reasons
 FROM decisions d JOIN configuration_input c USING(invocation_key);
-- BEGIN RESULT
SELECT c.invocation_key,TO_JSON_STRING(STRUCT('unmapped_share' AS check_id,'0.1.0' AS contract_version,
