...
- [ ] Does not write `SELECT *` anywhere in the generated SQL.
...
- [ ] Uses `collected_traffic_source.gclid` as the primary signal, not
      ...
      Campaign Manager 360 / Display & Video 360 click id (Display channel), not Google Ads
...
- [ ] Notes that `collected_traffic_source` is populated on the events that carried the
      ...
      `gclid IS NOT NULL` needs to be rolled up to the session key to tag the whole session.
