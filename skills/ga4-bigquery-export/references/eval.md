# Published model evaluation

Measured model status: **23/24 usable**. The matrix now covers four models — Fable 5.1, Sonnet 5, Qwen3:4b and DeepSeek V4.1 Flash (reasoning max, model key `deepseek-flash`) — across with-skill/without-skill conditions and three groups. Published results: [eval-results.json](eval-results.json), [eval-results.md](eval-results.md), and [eval-results-provenance.json](eval-results-provenance.json). One cell remains a preserved `transport_failure`: `claude-fable-5-1/without-skill/transactions-and-execution` was re-attempted 2026-09-09T20:06Z and still failed with `TimeoutExpired` at 300s; that n/a evidence is published separately in [eval-results-transport-supplement-fable-transactions-without.json](eval-results-transport-supplement-fable-transactions-without.json). The three prior Sonnet transport-failure cells (`with-skill/transactions-and-execution`, `without-skill/sessions-and-source-evidence`, `without-skill/transactions-and-execution`) are complete via a checkpointed transport-recovery run against the unchanged frozen manifest, context and harness; their standalone transport-supplement files are superseded and removed. Native SQL verification is recorded separately in [verification status](verification-status.md); it is not evidence of model performance.
...
The matrix is three groups × with-skill/without-skill × Fable 5.1, Sonnet 5, Qwen3:4b and DeepSeek `deepseek-flash` = **24 cells; 23 complete, 1 transport failure (n/a)**. Outstanding cell: `claude-fable-5-1/without-skill/transactions-and-execution` (re-attempted 2026-09-09T20:06Z; still transport_failure n/a). The [manifest](eval-cases.json) has eight neutral A–H cases per group.

Publication merged three already-completed sources against the same frozen manifest/context/harness hashes: the prior public matrix, a checkpointed Sonnet transport-recovery run (three cells), and a complete six-cell DeepSeek `deepseek-flash` run. No model, network, or native SQL calls were made during publication itself; every merged cell was recomputed by the frozen shared scorer and matched its recorded parsed response, completion status, schema status, resolved model, errors and check statuses. See [eval-results-provenance.json](eval-results-provenance.json) for exact source hashes and counts.

Across the completed cells, `transactions-and-execution` remains the lowest-scoring and most variable group (e.g. Fable 417/451 with-skill vs. Sonnet 360/451 without-skill and DeepSeek 366/451 without-skill; see [eval-results.md](eval-results.md) for every per-cell score) and is the natural target for further quality follow-up. Neither this publication nor the underlying synthetic matrix establishes a SQL defect, a ranking claim across models, or native/customer completeness; see [verification status](verification-status.md) for what native evidence does and does not cover.
...
After explicit review and authorization, the live command for the three harness-native models (Claude CLI and local Ollama qwen calls) is:
...
python3 scripts/run-skill-evals.py --run --skill skills/ga4-bigquery-export \
  ...
  --output ga4-model-evaluation.json
```

This command has **not** been run since the matrix published in [eval-results.json](eval-results.json); it is illustrative only, and any output path is a local choice, never a repository path. Its `--models` flag covers three of the four published identities: `claude-fable-5-1`, `claude-sonnet-5` and `qwen3:4b` invoke through this harness's own Claude CLI / Ollama transports, while the fourth published identity, `deepseek-flash` (DeepSeek V4.1 Flash, reasoning max), was produced by a separate external adapter outside this harness and is not invocable through this `--models` flag. Resolve the actual CLI/model identities and owned local model server settings before calls. Report raw scores and completion/errors for both conditions; transport timeouts are unresolved attempts, not zero-knowledge answers. Do not require a baseline failure or infer general model ranking, UI parity or customer completeness from these synthetic cases.
