...
try {
  ...
  for (const artifact of artifacts) {
    ...
    const stale = artifact.endsWith('.sql') ? original.replace("const TAXONOMY_VERSION =", "const STALE_TAXONOMY_VERSION =") : original + '\nSTALE\n';
  ...
  console.log('PASS standalone generator isolation and drift detection for all 6 shared artifacts');
