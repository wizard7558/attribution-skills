...
| [`first-party-pixel`](skills/first-party-pixel) | 0.2.0 | Owner-controlled pixel, collector, and PostgreSQL session/daily reporting views | Deterministic tests pass; harness manifest authored; live matrix published 18/18; hosted adapter smoke pending |
| [`clickstream-identity-stitching`](skills/clickstream-identity-stitching) | 0.1.0 | Identity dedupe, non-destructive graph, and verified webhook resolution | Deterministic suite passes; Sonnet supplements published (4/4); Fable graph supplement published (2/2); Qwen without-skill graph published (574/1188); Qwen with-skill truncated (n/a) |
...
Deterministic test results and model behavior evaluations are separate evidence. Published matrices for taxonomy (18/18), GA4 (14/18), pixel (18/18), tripwires/MMM/funnel/CRM skills, and identity supplements are honest about transport and truncation gaps. The Qwen with-skill identity-graph cell still truncates at the 8192-token output cap and remains an outstanding blocker, not a knowledge score.
...
| Identity corrected graph (Fable graph ×2 complete and published in [fable graph supplement](skills/clickstream-identity-stitching/references/eval-results-fable-graph-supplement.md); Qwen without-skill complete and published in [qwen without-skill graph supplement](skills/clickstream-identity-stitching/references/eval-results-qwen-graph-without-supplement.md); Qwen with-skill still truncated n/a from prior [qwen graph supplement](skills/clickstream-identity-stitching/references/eval-results-qwen-graph-supplement.md)) | `clickstream-identity-stitching` | Decide truncation policy for with-skill cell; rerun only never-run or failed cells on manifest `71a0c119`; original matrix published on prior manifest `9e2b5e6e` |
| Audit live matrix (18 cells) | `attribution-audit` | In flight 2026-09-09 in the same private live batch as pixel; publish redacted results when terminal |
| Hosted pixel adapter smoke | `first-party-pixel` | **Blocked:** Vercel CLI authenticated (`wizard7558`) and Supabase projects exist, but none are disposable teardown targets for this catalog. Do not reuse customer projects (`Cortex Analytics`, `og1-colleges`, `SSP`, etc.). Rerun when a disposable Vercel+Neon (or Vercel+Supabase/Cloudflare) pair is provisioned: deploy adapter → collector roundtrip → teardown → record evidence under a private Downloads journal, then publish a redacted summary |
