...
npx skills add wizard7558/attribution-skills --skill budget-scenario-planning
```
...
| [`ga4-bigquery-export`](skills/ga4-bigquery-export) | 0.2.0 | Source-scoped GA4 BigQuery sessions and daily channel metrics from raw export events | Deterministic fixtures pass; model matrix published 23/24 usable across four models (Fable 5.1, Sonnet 5, Qwen3:4b, DeepSeek `deepseek-flash`); 1 Fable transport failure preserved n/a |
...
Deterministic test results and model behavior evaluations are separate evidence. Published matrices for taxonomy (18/18), GA4 (23/24), pixel (18/18), audit (18/18), tripwires/MMM/funnel/CRM skills, and identity supplements are honest about transport gaps. Identity corrected-graph supplements are complete on manifest `71a0c119`; the prior Qwen with-skill 8192 attempt remains archived as historical n/a evidence.

## Outstanding blockers

| Blocker | Skill | Owner action |
| --- | --- | --- |
| GA4 Fable transport failure (`transactions-and-execution` without-skill re-attempted 2026-09-09T20:06Z; still `TimeoutExpired` n/a at 300s, preserved in [transport supplement](skills/ga4-bigquery-export/references/eval-results-transport-supplement-fable-transactions-without.md)). The three Sonnet transport-failure cells are resolved: a checkpointed transport-recovery run completed them against the unchanged frozen manifest/harness, and their standalone supplements were removed as superseded. | `ga4-bigquery-export` | Escalate timeout policy or rerun if the Fable cell fails again; never resume whole artifact under changed manifest |
| Catalog residual gaps | all twelve | Shippable with honest residuals: GA4 1× Fable `TimeoutExpired` n/a preserved (not every transport error is resolved). Hosted pixel adapter smoke complete. No fabricated scores. |
